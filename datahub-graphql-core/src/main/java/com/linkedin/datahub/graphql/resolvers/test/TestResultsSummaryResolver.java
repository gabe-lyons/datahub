...
public class TestResultsSummaryResolver implements DataFetcher<CompletableFuture<TestResultsSummary>> {
  ...
  public TestResultsSummaryResolver(@Nonnull final EntitySearchService entitySearchService) {
    this.entitySearchService = Objects.requireNonNull(entitySearchService, "entitySearchService must not be null");
  }

  @Override
  public CompletableFuture<TestResultsSummary> get(DataFetchingEnvironment environment) throws Exception {
  ...
  private long getResultsCount(@Nonnull final Urn testUrn, @Nonnull final String fieldName) {
    try {
      final Map<String, Long> aggregations = this.entitySearchService.aggregateByValue(
          null,
          fieldName,
      ...
      log.error(String.format("Failed to fetch the test result counts for test with urn %s! Returning 0 results.", testUrn), e);
      return 0L;
    }
  }
