...
@Slf4j
public class UpdateSlaAssertionResolver implements DataFetcher<CompletableFuture<Assertion>> {

  private final AssertionService _assertionService;

  public UpdateSlaAssertionResolver(@Nonnull final AssertionService assertionService) {
  ...
  public CompletableFuture<Assertion> get(final DataFetchingEnvironment environment) throws Exception {
    ...
    final UpdateSlaAssertionInput
        input = ResolverUtils.bindArgument(environment.getArgument("input"), UpdateSlaAssertionInput.class);
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      if (AssertionUtils.isAuthorizedToEditAssertionFromAssertee(context, asserteeUrn)) {
        ...
        _assertionService.updateSlaAssertion(
            assertionUrn,
