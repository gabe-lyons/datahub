...
public class RaiseIncidentResolver implements DataFetcher<CompletableFuture<String>> {
  ...
  public CompletableFuture<String> get(DataFetchingEnvironment environment) throws Exception {
    ...
    final RaiseIncidentInput input = bindArgument(environment.getArgument("input"), RaiseIncidentInput.class);
  ...
  private IncidentInfo mapIncidentInfo(final RaiseIncidentInput input, final QueryContext context) throws URISyntaxException {
    final IncidentInfo result = new IncidentInfo();
    result.setType(IncidentType.valueOf(input.getType().name())); // Assumption Alert: This assumes that GMS incident type === GraphQL incident type.
    ...
    result.setEntities(new UrnArray(ImmutableList.of(Urn.createFromString(input.getResourceUrn()))));
    result.setCreated(new AuditStamp()
      .setActor(Urn.createFromString(context.getActorUrn()))
      .setTime(System.currentTimeMillis())
    );
    // Create the incident in the 'active' state by default.
    result.setStatus(new IncidentStatus()
  ...
  private boolean isAuthorizedToCreateIncidentForResource(final Urn resourceUrn, final QueryContext context) {
