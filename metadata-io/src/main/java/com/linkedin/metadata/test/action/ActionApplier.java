 ...
 *
 * It wraps a set of {@link Action} and invokes them accordingly when an action needs to be applied.
 */
@Slf4j
public class ActionApplier {
  ...
  public ActionApplier(@Nonnull final List<Action> applierList) {
    this.actionAppliers = applierList.stream().collect(Collectors.toMap(
   ...
   */
  public void apply(@Nonnull final ActionType type, @Nonnull final List<Urn> urns, @Nonnull final ActionParameters params) {
    ...
    throw new IllegalArgumentException(String.format("Unrecognized action applier with type %s provided!", type));
  }
}
