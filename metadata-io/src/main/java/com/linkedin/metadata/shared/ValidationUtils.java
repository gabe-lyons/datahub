...
public class ValidationUtils {
  ...
  public static SearchResult validateSearchResult(final SearchResult searchResult,
    ...
    try (Timer.Context ignored = MetricUtils.timer(ValidationUtils.class, "validateSearchResult").time()) {
  ...
  public static BrowseResult validateBrowseResult(final BrowseResult browseResult,
      @Nonnull final EntityService entityService) {
    try (Timer.Context ignored = MetricUtils.timer(ValidationUtils.class, "validateBrowseResult").time()) {
  ...
  public static ListResult validateListResult(final ListResult listResult, @Nonnull final EntityService entityService) {
    try (Timer.Context ignored = MetricUtils.timer(ValidationUtils.class, "validateListResult").time()) {
      if (listResult == null) {
        return null;
      }
      Objects.requireNonNull(entityService, "entityService must not be null");

      ListResult validatedListResult = new ListResult().setStart(listResult.getStart()).setCount(listResult.getCount()).setTotal(listResult.getTotal());

      UrnArray validatedEntities = listResult.getEntities().stream().filter(entityService::exists).collect(Collectors.toCollection(UrnArray::new));
  ...
  public static LineageSearchResult validateLineageSearchResult(final LineageSearchResult lineageSearchResult,
      @Nonnull final EntityService entityService) {
    try (Timer.Context ignored = MetricUtils.timer(ValidationUtils.class, "validateLineageResult").time()) {
      if (lineageSearchResult == null) {
        return null;
      }
      Objects.requireNonNull(entityService, "entityService must not be null");

      LineageSearchResult validatedLineageSearchResult = new LineageSearchResult().setMetadata(lineageSearchResult.getMetadata())
