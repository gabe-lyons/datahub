...
public class AnomalyInfoArgumentMatcher implements ArgumentMatcher<MetadataChangeProposal> {
  ...
  private boolean incidentInfoMatches(GenericAspect left, GenericAspect right) {
    AnomalyInfo leftProps = GenericRecordUtils.deserializeAspect(
        left.getValue(),
        "application/json",
        AnomalyInfo.class
    );

    AnomalyInfo rightProps = GenericRecordUtils.deserializeAspect(
        right.getValue(),
        "application/json",
        AnomalyInfo.class
    );
    ...
    if (leftProps.getStatus().hasProperties() && leftProps.getStatus().getProperties().hasAssertionRunEventTime()) {
      if (!leftProps.getStatus().getProperties().getAssertionRunEventTime().equals(rightProps.getStatus().getProperties().getAssertionRunEventTime())) {
        return false;
      }
    }

    if (leftProps.getSource().hasProperties() && leftProps.getSource().getProperties().hasAssertionRunEventTime()) {
      if (!leftProps.getSource().getProperties().getAssertionRunEventTime().equals(rightProps.getSource().getProperties().getAssertionRunEventTime())) {
        return false;
      }
    }
