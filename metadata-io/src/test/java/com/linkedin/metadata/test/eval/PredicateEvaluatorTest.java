...
public class PredicateEvaluatorTest {
  ...
  private Predicate createCompositePredicate(String operation, List<Predicate> childPredicates) {
    return Predicate.of(OperatorType.fromCommonName(operation), childPredicates
        .stream()
  ...
  public void testEvaluateAnyEqualsPredicate() {
    Predicate equalsRule = new Predicate(OperatorType.ANY_EQUALS,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
            new Operand(1, "values", new StringListLiteral(ImmutableList.of("value1", "value2"))))
    );
    ...
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("value1")));
    assertTrue((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));

    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("value3")));
    assertFalse((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));

    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("value1", "value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));

    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("value2")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));

    queryResponses = ImmutableMap.of(new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value1")));
    assertFalse((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));

    queryResponses = ImmutableMap.of(new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")),
        new TestQuery("testProperty3"), new TestQueryResponse(ImmutableList.of("value1")));
    assertFalse((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));

    // Is empty string
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertFalse((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));
    ...
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(Collections.emptyList()),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertFalse((boolean) _evaluator.evaluatePredicate(equalsRule, queryResponses));
  }
  ...
  public void testEvaluateRegexMatchPredicate() {
    Predicate regexMatchRule = new Predicate(OperatorType.REGEX_MATCH,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
            new Operand(1, "values", new StringListLiteral(ImmutableList.of("testtext.*", ".*testtext"))))
    );
    ...
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("testtext123")));
    assertTrue((boolean) _evaluator.evaluatePredicate(regexMatchRule, queryResponses));

    // Matches 1, multiple properties in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("testtext123", "value2")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(regexMatchRule, queryResponses));

    // Matches 2, multiple properties in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("testtext123", "123testtext")));
    assertTrue((boolean) _evaluator.evaluatePredicate(regexMatchRule, queryResponses));

    // No matching properties
    queryResponses = ImmutableMap.of(new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("testtext123")),
        new TestQuery("testProperty3"), new TestQueryResponse(ImmutableList.of("123testtext")));
    assertFalse((boolean) _evaluator.evaluatePredicate(regexMatchRule, queryResponses));
  ...
  public void testEvaluateStartsWithPredicate() {
    Predicate startsWithRule = new Predicate(OperatorType.STARTS_WITH,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
            new Operand(1, "values", new StringListLiteral(ImmutableList.of("test1", "test2"))))
    );
    ...
    // Matches 1, single property in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("test1something")));
    assertTrue((boolean) _evaluator.evaluatePredicate(startsWithRule, queryResponses));

    // Matches 1, multiple properties in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("test2something", "value2")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(startsWithRule, queryResponses));

    // Matches 2, multiple properties in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("test1something", "test2something")));
    assertTrue((boolean) _evaluator.evaluatePredicate(startsWithRule, queryResponses));

    // No matching properties
    queryResponses = ImmutableMap.of(new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("test1something")),
        new TestQuery("testProperty3"), new TestQueryResponse(ImmutableList.of("test2something")));
    assertFalse((boolean) _evaluator.evaluatePredicate(startsWithRule, queryResponses));
    ...
    // Is empty string + good value
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("", "test1")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(startsWithRule, queryResponses));
  ...
  public void testEvaluateContainsStrPredicate() {
    Predicate containsStrRule = new Predicate(OperatorType.CONTAINS_STR,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
            new Operand(1, "values", new StringListLiteral(ImmutableList.of("test1", "test2"))))
    );
    ...
    // Matches 1, single property in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("prefix-test1-suffix")));
    assertTrue((boolean) _evaluator.evaluatePredicate(containsStrRule, queryResponses));

    // Matches 1, multiple properties in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("prefix-test1-suffix", "value2")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(containsStrRule, queryResponses));

    // Matches 2, multiple properties in response
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("prefix-test1-suffix", "prefix-test2-suffix")));
    assertTrue((boolean) _evaluator.evaluatePredicate(containsStrRule, queryResponses));

    // No matching properties
    queryResponses = ImmutableMap.of(new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("prefix-test1-suffix")),
        new TestQuery("testProperty3"), new TestQueryResponse(ImmutableList.of("prefix-test2-suffix")));
    assertFalse((boolean) _evaluator.evaluatePredicate(containsStrRule, queryResponses));
  ...
  public void testEvaluateExistsPredicate() {
    Predicate existsRule = new Predicate(OperatorType.EXISTS,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty"))));
    ...
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("", "value")),
        new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("value3")));
    assertTrue((boolean) _evaluator.evaluatePredicate(existsRule, queryResponses));
  ...
  public void testEvaluateGreaterThanPredicate() {
    Predicate greaterThanRule = new Predicate(OperatorType.GREATER_THAN,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
    ...
    // Is greater than
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("10")));
    assertTrue((boolean) _evaluator.evaluatePredicate(greaterThanRule, queryResponses));

    // Some greater than
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("10", "1")));
    assertTrue((boolean) _evaluator.evaluatePredicate(greaterThanRule, queryResponses));

    // Is equal to
    queryResponses = ImmutableMap.of(new TestQuery("testProperty2"), new TestQueryResponse(ImmutableList.of("5")));
    assertFalse((boolean) _evaluator.evaluatePredicate(greaterThanRule, queryResponses));

    // Is less than
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("4")));
    assertFalse((boolean) _evaluator.evaluatePredicate(greaterThanRule, queryResponses));
    ...
    Predicate badRule = new Predicate(OperatorType.GREATER_THAN,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
    ...
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("test")));
    assertFalse((boolean) _evaluator.evaluatePredicate(greaterThanRule, queryResponses));
  }

  @Test
  public void testEvaluateLessThanPredicate() {
    Predicate lessThanRule = new Predicate(OperatorType.LESS_THAN,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
            new Operand(1, "values", new StringListLiteral(ImmutableList.of("5"))))
    );
    ...
    // Empty string + good value
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("", "1")));
    assertTrue((boolean) _evaluator.evaluatePredicate(lessThanRule, queryResponses));

    // Bad input operand -> not a number
    Predicate badRule = new Predicate(OperatorType.LESS_THAN,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty")),
            new Operand(1, "values", new StringListLiteral(ImmutableList.of("test"))))
    );
  ...
  public void testEvaluateIsTruePredicate() {
    Predicate isTrueRule = new Predicate(OperatorType.IS_TRUE,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty"))));
    ...
    // Contains value true
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("true")));
    ...
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("true", "false")));
    assertTrue((boolean) _evaluator.evaluatePredicate(isTrueRule, queryResponses));

    // Contains false
    queryResponses = ImmutableMap.of(new TestQuery("testProperty"), new TestQueryResponse(ImmutableList.of("false")));
    assertFalse((boolean) _evaluator.evaluatePredicate(isTrueRule, queryResponses));
  ...
  public void testEvaluateIsFalsePredicate() {
    Predicate isFalseRule = new Predicate(OperatorType.IS_FALSE,
        ImmutableList.of(
            new Operand(0, "query", new Query("testProperty"))));
  ...
  public void testEvaluateAndPredicate() {
    ...
    Predicate andRule = new Predicate(OperatorType.AND,
        ImmutableList.of(
            new Operand(0, null, greaterThanRule),
            new Operand(1, null, equalsRule)));
    ...
    queryResponses = ImmutableMap.of(
  ...
  public void testEvaluateOrPredicate() {
    ...
    Predicate orRule = new Predicate(OperatorType.OR,
        ImmutableList.of(
            new Operand(0, null, greaterThanRule),
            new Operand(1, null, equalsRule)));
  ...
  public void testEvaluateNotPredicate() {
    ...
    Predicate notRule = new Predicate(OperatorType.NOT,
        ImmutableList.of(
            new Operand(0, null, greaterThanRule)));
  ...
  public void testEvaluateNestedPredicates() {
    Predicate equalsRule1 = new Predicate(OperatorType.fromCommonName("equals"),
      ImmutableList.of(
          new Operand(0, "query", new Query("query1")),
          new Operand(1, "values", new StringListLiteral(ImmutableList.of("value1", "value2"))))
    );
    Predicate equalsRule2 = new Predicate(OperatorType.fromCommonName("equals"),
      ImmutableList.of(
          new Operand(0, "query", new Query("query2")),
          new Operand(1, "value", new StringListLiteral(ImmutableList.of("value3"))))
    );

    Predicate existsRule = new Predicate(OperatorType.fromCommonName("exists"),
    ...
    Predicate complexRule = createCompositePredicate("or",
