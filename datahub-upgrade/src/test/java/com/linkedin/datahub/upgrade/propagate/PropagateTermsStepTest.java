...
public class PropagateTermsStepTest {
  private static final Urn DATASET_URN1 = UrnUtils.getUrn("urn:li:dataset:(urn:li:dataPlatform:hive,SampleHiveDataset,PROD)");
  private static final Urn DATASET_URN2 = UrnUtils.getUrn("urn:li:dataset:(urn:li:dataPlatform:hive,SampleHiveDataset2,PROD)");
  ...
  public void testExecutable() {
    ...
    PropagateTermsStep propagateTermsStep = new PropagateTermsStep(entityService, entitySearchService);
    ...
    List<String> expectedLines = List.of("Starting term propagation (Run ID: term_propagation_1685663328210_KhOv)...",
    ...
    assertTrue(report.lines().get(expectedLines.size() - 1).startsWith("Finished term propagation (Run ID:"));
  ...
  private static void configureEntitySearchServiceMock(final EntitySearchService mockSearchService) {
    ...
    Mockito.when(mockSearchService.filter(Mockito.eq(Constants.DATASET_ENTITY_NAME),
        Mockito.any(),
        Mockito.eq(null),
        Mockito.eq(0),
        Mockito.eq(5000))).thenReturn(new SearchResult().setNumEntities(0).setEntities(new SearchEntityArray()));
  }
}
